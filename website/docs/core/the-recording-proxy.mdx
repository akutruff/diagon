...
You create recording proxies via  [`createRecordingProxy`](create-recording-proxy.mdx).  This only needs to be called on the root of your object state tree.
...
Some functions like [`recordPatches`](/record-patches.mdx) will automatically ensure that a recording proxy exists or is created before calling your code. 
