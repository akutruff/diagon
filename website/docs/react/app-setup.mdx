...
At the top level of your app, you need a [`SubscriptionContext`](/react/subscription-context.mdx) provider that will enable support for all of Diagon's hooks.

That provider is initialized with [`useSubscriptionContextValue`](/react/use-subscription-context-value.mdx) which holds your root state and tracks object subscriptions in your app. 
