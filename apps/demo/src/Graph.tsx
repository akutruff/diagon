...
import { useMutator, useProjectedSnapshot, useSnapshot } from 'diagon-react';
...
export const GraphNodeComponent: FC<GraphNodeComponentProps> = React.memo(({ node }) => {
    const [data, parentData, grandParent, children] = useSnapshot(node, node => [node.data, node.parent?.data, node.parent?.parent, elements(node.children)]);
    const isLastChild = useProjectedSnapshot(node,
...
export const Graph: FC = React.memo(() => {
    ...
    const root = useSnapshot(state, state => state.root);
